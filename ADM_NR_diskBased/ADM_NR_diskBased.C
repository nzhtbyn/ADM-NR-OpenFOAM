...
#include "cellSet.H" // Inserted this line
    ...
    switch (monitorMethod_)
    {
        ...
        case monitorMethodType::CELLSET:
        {
            Info<< "    - selecting cells using cellSet "
                << cellSetName() << endl;   // Changed cellSetName_ to cellSetName()

            monitorCells_ = cellSet(mesh_, cellSetName()).sortedToc();  // Changed cellSetName_ to cellSetName()
            break;
        }
        default:
        {
